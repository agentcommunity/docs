...
// Simple MDX components for blog (not dependent on DocsLayout)
function getBlogMDXComponents(): MDXComponents {
  return {
    // Basic HTML components without Fumadocs UI dependencies
    h1: ({ children, ...props }) => (
    ...
    h3: ({ children, ...props }) => (
      <h3 className="text-xl font-semibold mb-2" {...props}>{children}</h3>
    ),
    p: ({ children, ...props }) => (
    ...
    pre: ({ children, ...props }) => (
      <pre className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg overflow-x-auto mb-4" {...props}>{children}</pre>
    ),
  };
}
...
export default async function BlogPage(props: {
  ...
  // Render dynamic index listing for blog root
  if (isRoot) {
    const pages = (blogSource.getPages() as unknown as LoadedBlogPage[])
      .filter((p) => p.slugs.join('/') !== 'index');
        ...
        <ul className="space-y-6">
          {sorted.map((p) => {
            ...
            const dateLabel = publishedAt
              ? publishedAt.toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: '2-digit' })
              : undefined;
  ...
  // For API calls, we need to handle the root case differently
  // When slug is empty/undefined, we should use 'index' or handle it specially
      ...
      <article className="prose prose-neutral dark:prose-invert max-w-none">
        <header className="mb-8">
          ...
          {pageData.description && (
            <p className="text-lg md:text-xl text-muted-foreground mb-4">{pageData.description}</p>
          )}
          <div className="flex flex-row gap-2 items-center border-b pt-2 pb-6">
            <LLMCopyButton markdownUrl={`/api/mdx/blog/${apiSlug}`} />
            <ViewOptions
              markdownUrl={`/api/mdx/blog/${apiSlug}`}
              githubUrl={`https://github.com/agentcommunity/docs/blob/main/content/blog/${page.slugs.join('/')}.mdx`}
            />
          </div>
        </header>
      ...
      <Script id="ld-article" type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            datePublished: (() => {
              if (pageData.date instanceof Date) return pageData.date.toISOString();
              if (typeof pageData.date === 'string') {
                const d = new Date(pageData.date);
                return Number.isNaN(d.getTime()) ? undefined : d.toISOString();
              }
              return undefined;
            })(),
            dateModified: pageData.lastModified ?? undefined,
            author: pageData.author ? { '@type': 'Person', name: pageData.author } : undefined,
            mainEntityOfPage: {
              '@type': 'WebPage',
...
export async function generateMetadata(props: {
  ...
  return {
    ...
    description: pageData.description,
    alternates: { canonical: `${base}/blog/${slugPath}` },
    openGraph: {
      type: 'article',
      title: pageData.title,
      description: pageData.description,
      url: `${base}/blog/${slugPath}`,
    },
  };
} 