...
export function ViewOptions({ markdownUrl, githubUrl }: { markdownUrl: string; githubUrl: string }) {
  const items = useMemo(() => {
    ...
    async function copyMarkdownThenOpen(href: string) {
      try {
        const urls: string[] = [];
        if (typeof window !== 'undefined') urls.push(`${window.location.pathname}.mdx`);
        urls.push(fullMarkdownUrl.toString());
        ...
        for (const url of urls) {
          ...
          try {
            ...
            if (!res.ok) {
              if (res.status === 404 && url.endsWith('.mdx')) continue;
        ...
        // If copy fails, still open the link
        window.open(href, '_blank', 'noopener,noreferrer');
      }
    }
    ...
    return [
      ...
      },
      {
        ...
        onClick: () => copyMarkdownThenOpen(`https://chatgpt.com/?${new URLSearchParams({ hints: 'search', q })}`),
        icon: (
      ...
      },
      {
        ...
        onClick: () => copyMarkdownThenOpen(`https://claude.ai/new?${new URLSearchParams({ q })}`),
        icon: <MessageCircleIcon />,
      },
      {
        title: 'Open in T3 Chat',
        href: `https://t3.chat/new?${new URLSearchParams({ q })}`,
        onClick: () => copyMarkdownThenOpen(`https://t3.chat/new?${new URLSearchParams({ q })}`),
        icon: <MessageCircleIcon />,
      },
    ];
