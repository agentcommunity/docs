...
But it has a hidden, dangerous flaw. It’s the digital equivalent of leaving a handwritten note on the front door of a bank that says, "New deposit procedure: please slide cash and checks under the floorboard in the alley."
...
This is where Agent Identity & Discovery (AID) offers a more robust, foundational solution. AID doesn't leave instructions on the page. It publishes an address in the internet's most trusted, decentralized address book: DNS.

The entire mechanism is a single, machine-readable `TXT` record.

`_agent.example.com. IN TXT "v=aid1 p=mcp u=https://api.example.com/mcp"`
...
2.  **It's Deterministic.** An AID client isn't interpreting natural language. It's parsing a `u` field and a `p` field. The attack surface for prompt injection is virtually eliminated because there's no ambiguous prose to inject into. The logic is handled by a simple, deterministic parser, not a complex LLM.
...
Original Vercel LLMs.txt article at the blog: [`<script type="text/llms.txt">`](https://vercel.com/blog/a-proposal-for-inline-llm-instructions-in-html)

---

**Related Reading:**

- [AID Specification](https://aid.agentcommunity.org)
- [Design Rationale](https://aid.agentcommunity.org)
- [The Missing MX Record for the Internet of Agents](https://blog.agentcommunity.org/missing_record)
