...
date: 2026-03-16
tags: [dns, svcb, srv, txt, aid, standards, agent-discovery, update]
---
...
It goes deeper than Azure's managed DNS service. Windows DNS Server itself — the authoritative server software that enterprises run on-premises — [doesn't support SVCB or HTTPS record types either](https://www.simonpainter.com/svcb-https-records/). Simon Painter, a Microsoft MVP, documented this in March 2026: if you're running Windows DNS infrastructure and want to publish SVCB records, the workaround is to stand up a separate BIND instance. That's not a realistic ask for most enterprise DNS teams.
...
**\*Update (March 23, 2026):** GoDaddy's engineering team reached out to clarify that their DNS API backend already accepts HTTPS records with arbitrary SvcParams in ServiceMode — the limitation I tested was the web UI at dcc.godaddy.com, which presents a simplified form that doesn't expose priority or ServiceMode parameters. Their backend database stores records as type-code-plus-rdata, so SVCB support is a configuration addition (adding one entry to their type table), not an architectural change. They also noted that UI updates to expose ServiceMode parameters are currently in testing. This is good news for the SVCB ecosystem. The core argument stands — developers hitting the GoDaddy web panel today still can't publish a ServiceMode record — but the gap is closing faster than the table above suggests, and credit to GoDaddy for actively working on it.
