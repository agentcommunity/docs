...
<Cards>
  <Card
    href="https://aid.agentcommunity.org/docs"
    title="Agent Identity & Discovery (AID)"
    description="DNS for Agents — specification, rationale, and tooling (aid.agentcommunity.org)"
    icon={
...
### First Work Item: Agent Identity & Discovery (AID)

Our starting task is **AID**, a lightweight DNS‑based protocol that lets any client discover an agent's endpoint and config with a single query. AID wipes out the "where is the API" problem and lays the groundwork for richer registry services. AID documentation now lives at [aid.agentcommunity.org/docs](https://aid.agentcommunity.org/docs).
